package com.and3r.octoprint.javadisplay.ui.utils;
...
// https://stackoverflow.com/a/38227129
...
public class TouchScroll extends JScrollPane implements MouseListener,   MouseMotionListener {
    ...
    public static TouchScroll generateSroll(JComponent component){
        TouchScroll touchScroll = new TouchScroll(component);
        touchScroll.setOpaque(false);
        touchScroll.getViewport().setOpaque(false);
        touchScroll.setBorder(BorderFactory.createEmptyBorder());
        touchScroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
        touchScroll.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        touchScroll.getViewport().addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                System.out.println(touchScroll.getVerticalScrollBar().getMaximum()+ "##" + touchScroll.getViewport().getHeight());
            }
        });
        return touchScroll;
    }
